...
function RecentAppointment({ patients }: TProps) {
  ...
  const patientsTotalFees = () => {
    const total = finshAppointments.reduce((acc, curr) => {
      return acc + Number(curr.total_fees);
    }, 0);
    return total;
  };
          ...
          {finshAppointments.length === 0 ? (
            ...
            >
              <span className="mb-2 flex justify-end">
                Total: {patientsTotalFees()}
              </span>
              {[...finshAppointments]
                .sort((b, a) => a.time_stamp.localeCompare(b.time_stamp))
                .map((app) => {
